...
struct ImuData {
  double timestamp{0};  // in nanoseconds
  gtsam::Vector3 linear_acceleration{0, 0, 0};
  gtsam::Vector3 angular_velocity{0, 0, 0};

  // Default constructor
  ImuData() = default;
  void print() { std::cout << timestamp << ", " << linear_acceleration << ", " << angular_velocity << std::endl; }
};
...
struct VisualOdometryData {
  ...
  void print() {
    std::cout << timestamp << ", " << pose.translation().x() << ", " << pose.translation().y() << ", "
              << pose.translation().y() << ", " << pose.rotation() << std::endl;
  }
};
...
int main(int argc, char const *argv[]) {
  if (argc != 3) {
    std::cerr << "Usage: " << argv[0] << " <imu_file.txt> <vo_file.txt>" << std::endl;
    return 1;
  }

  // Load IMU and VO data
  std::vector<ImuData> imu_data = loadImuData(argv[1]);
  std::vector<VisualOdometryData> vo_data = loadVoData(argv[2]);

  std::cout << "size imu: " << imu_data.size() << std::endl;
  std::cout << "size vo: " << vo_data.size() << std::endl;
  ...
  // Matrix33 bias_acc_cov = Matrix33::Identity(3,3) *
  // pow(accel_bias_rw_sigma,2); Matrix33 bias_omega_cov =
  // Matrix33::Identity(3,3) * pow(gyro_bias_rw_sigma,2); Matrix66
  // bias_acc_omega_int = Matrix::Identity(6,6)*1e-5; // error in the bias used
  // for preintegration
  ...
  for (auto iter_odom = vo_data.begin() + 1; iter_odom != vo_data.end(); ++iter_odom) {
    ...
    for (auto iter_imu = imu_data.begin(); iter_imu->timestamp <= iter_odom->timestamp; ++iter_imu) {
      ...
      //   imu << -iter_imu->linear_acceleration.x,
      //   iter_imu->linear_acceleration.y, -iter_imu->linear_acceleration.z,
      //       -iter_imu->angular_velocity.x, iter_imu->angular_velocity.y,
      //       -iter_imu->angular_velocity.z;
    ...
    // noiseModel::Isotropic::Sigma(3, 1, 0); GPSFactor
    // gps_factor(X(correction_count), Point3(odom(0), odom(1), odom(2)),
    // correction_noise); graph.add(gps_factor);
    ...
    auto vo_noise_model = gtsam::noiseModel::Isotropic::Sigma(6, 0.0001);  // Tune sigma based on VO accuracy
    ...
    VisualOdometryData tmp;
    tmp.timestamp = iter_odom->timestamp;
    tmp.pose = prev_state.pose();
    optimized_data.push_back(tmp);
  ...
  for (const auto &pose_data : optimized_data) {
    const auto pose_cam = T_imu_cam_.inverse() * pose_data.pose * T_imu_cam_;
    const auto &quat = pose_cam.rotation().toQuaternion();
    const auto &trans = pose_cam.translation();
